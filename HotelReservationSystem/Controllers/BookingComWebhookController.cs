...
public class BookingComWebhookController : ControllerBase
{
    ...
    private readonly IOptions<BookingComConfiguration> _config;
    ...
    public BookingComWebhookController(
        ...
        IOptions<BookingComConfiguration> config)
    {
    ...
    /// <summary>
    /// Verifies HMAC-SHA256 signature of a webhook payload.
    /// </summary>
...
public async Task<IActionResult> HandleWebhook(CancellationToken cancellationToken = default)
{
    try
    {
        ...
        var signature = Request.Headers["X-Booking-Signature"].FirstOrDefault();
        ...
        // If signature is missing, return 401 unauthorized
        if (string.IsNullOrEmpty(signature))
        {
        ...
        // Verify signature
        if (!VerifySignature(xmlPayload, signature, _config.Value.WebhookSecret))
        {
        ...
        // Process the webhook
        _logger.LogInformation("Webhook received for Booking.com");
